...
class FamilyServiceTest {
    ...
    /**
     * Prueba el manejo de errores durante la creación
     * Verifica que se manejen correctamente las excepciones
     */
    @Test
    @DisplayName("Debe manejar errores durante la creación de familia")
    void createFamily_WithError_ShouldReturnError() {
        // Given - Simulación de error en el repository
        testFamilyDTO.setBasicService(testBasicService);
        testFamilyDTO.setHousingDetails(testHousingDetails);
        
        when(housingServiceClient.createBasicService(testBasicService))
                .thenReturn(Mono.just(testBasicService));
        when(housingServiceClient.createHousingDetails(testHousingDetails))
                .thenReturn(Mono.just(testHousingDetails));
        when(familyMapper.toEntity(testFamilyDTO)).thenReturn(testFamily);
        when(familyRepository.save(any(Family.class)))
                .thenReturn(Mono.error(new RuntimeException("Database error")));

        // When & Then - Verificación del manejo de errores
        StepVerifier.create(familyService.createFamily(testFamilyDTO))
                .expectErrorMatches(throwable -> 
                    throwable instanceof RuntimeException &&
                    throwable.getMessage().contains("Error durante la creación de la familia")
                )
                .verify();
    }
